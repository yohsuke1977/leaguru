...
- 伏字・雛形（〇〇／○○／△△／[〜を入力] 等）は使わない。例示が必要なら具体的に書く（例：「Aチーム」「毎週日曜の朝」「LINEのノート機能」など）。
- 本文は日本語。2000〜3500字程度。
- 出力は必ず次の2ブロック形式のみ（前後に説明文やコードフェンスを付けない）。
  1行目に「---META---」、その次の行に1行JSON、次に「---BODY---」、その後に本文Markdown（生のまま）:
---META---
{"slug":"英小文字ハイフンのみ(既存と重複しない・内容を表す)","title":"28〜40字","description":"70〜140字(検索スニペット用・記事の要約)","tags":["2〜4個"]}
---BODY---
（ここに上記構造のMarkdown本文。frontmatterや # タイトルは含めない・結論ブロックから始める。改行や記号はそのままでよい）`;
...
- 上記の既存記事で既に詳しく扱っているトピック（例：成績の計算方法、規定打席の決め方、エクセル管理の限界、日程の組み方 等）は、この記事では詳述しない。必要なら1文で触れて [リンクテキスト](/blog/該当slug) 形式のMarkdownリンクで誘導する（本文中に自然に内部リンクを1〜3個。リンク先は必ず上記一覧に実在するslugのみ）。
- slugは既存と重複させない。
...
async function callClaude(feedback) {
  ...
  // ---META--- の1行JSON と ---BODY--- 以降の生Markdown に分けて取り出す（本文をJSONに入れない＝エスケープ事故回避）
  const bodyIdx = text.indexOf('---BODY---');
  if (bodyIdx === -1) throw new Error('出力に ---BODY--- マーカーが無い');
  const metaPart = text.slice(0, bodyIdx);
  const body = text.slice(bodyIdx + '---BODY---'.length).replace(/^\s*\n/, '').trimEnd();
  const jsonStr = metaPart.slice(metaPart.indexOf('{'), metaPart.lastIndexOf('}') + 1);
...
function gate(a) {
  ...
  // プレースホルダ（AIが埋め忘れた雛形）検出。普通の日本語散文を誤検知しないよう厳密に。
  const placeholders = [
    /TODO|FIXME|Lorem\s*ipsum|XXX+/i,   // 英語系の未記入マーカー
    /[〇○]{2,}/,                         // ○○ = 未記入
    /\{\{[^}]*\}\}/,                     // {{template}}
    /\[[^\]]*(?:入力|記入|未定|チーム名|ここに\S*記入)[^\]]*\]/, // [〜を入力] 等の角括弧雛形
    /（[^）]*(?:ここに\S*記入|ここに\S*入力|要記入|要入力|記入してください)[^）]*）/,
  ];
  const hit = placeholders.map((re) => body.match(re)).find(Boolean);
  if (hit) errs.push(`プレースホルダ残存: "${hit[0]}"`);
  // 内部リンク検証: ①[/blog/x] だけの壊れ書式 ②実在しないslugへのリンク
  const broken = body.match(/(?<!\]\()\[\/blog\/[a-z0-9-]+\](?!\()/);
  if (broken) errs.push(`内部リンクの書式が壊れている ${broken[0]} → [リンクテキスト](/blog/slug) 形式にすること`);
  for (const m of body.matchAll(/\]\(\/blog\/([a-z0-9-]+)\)/g)) {
    if (!existingSlugs.has(m[1])) errs.push(`存在しない記事への内部リンク /blog/${m[1]} → 実在する記事に変えるか削除すること`);
  }
  return errs;
}
