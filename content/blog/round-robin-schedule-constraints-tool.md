...
## 2回総当たり（ホーム＆アウェー）にする場合の注意点

2回総当たりを「1回目・2回目とも同じ組み合わせをもう一度組むだけ」と考えると、あとで不公平感が出ることがあります。整理しておきたいのは次の2点です。

- **会場の偏りをならす**：草野球リーグの多くは「ホーム球場」という概念が薄く、両チームとも同じ第三の球場を使うケースが多いですが、もし片方のチームの地元球場を使う運用なら、1回目・2回目で会場を入れ替える（1回目はAチーム側の球場、2回目はBチーム側の球場）のが基本です。
- **試合順を離す**：同じ2チームが2週連続で当たると消化試合感が出やすいため、1回目と2回目の対戦はシーズン前半・後半に分けて配置するのが実務的です。輪番方式（ベルガー法）で組んだ場合、後半戦は前半戦の対戦順をそのまま反転させると、機械的に「会場入れ替え・間隔確保」の両方を満たしやすくなります。
...
### Q. 2回総当たり（ホーム＆アウェー）で組むとき、1回目と2回目はどう配置すればよい？

同じ2チームの対戦が2週連続にならないよう、1回目の総当たりをシーズン前半、2回目をシーズン後半に分けて配置するのが基本です。輪番方式（ベルガー法）で組んだ場合は、後半戦の対戦順を前半戦の逆順にするだけで、間隔を空けつつ会場条件が偏っている場合の入れ替えもしやすくなります。
