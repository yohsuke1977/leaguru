  ...
  return (
    <LegalLayout title="利用規約">
      ...
      <LegalSection title="第3条（料金および支払い）">
        <p>利用料金は、リーグの登録チーム数に応じた年額（税込）です。{PLANS.map(pl => `${pl.teamsLabel}は${pl.priceLabel}`).join('、')}。チーム数はご契約時点の数で判定し、契約期間中に増減しても当該年度の料金は変更されません。更新時には、その時点のチーム数にもとづき料金を見直します。契約期間中にチーム数が上限を超える場合は上位プランへの変更が必要となり、その際は残り期間分の差額を日割りで請求します。下位プランへの変更は更新時にのみ行われます。初回は30日間の無料トライアル期間を設けており、トライアル期間中は料金が発生しません。トライアル終了後、自動的に有料プランへ移行し年額料金が請求されます。決済はStripeを通じたクレジットカード払いにより行います。支払い完了後の返金は原則として承っておりませんが、当サービスの重大な瑕疵による場合はこの限りではありません。</p>
      </LegalSection>
